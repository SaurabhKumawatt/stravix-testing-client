    ...
    try {
      ...
      const options = {
        ...
        handler: function (response) {
          // ✅ Razorpay response received, redirect user to dashboard
  ...
  useEffect(() => {
    const loadCourses = async () => {
      try {
        ...
        setCourses(all);
        if (!selectedCourse && all.length) setSelectedCourse(all[0]._id);
      } catch (err) {
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {courses.map((course) => (
                <label key={course._id}
                  className={`relative border p-3 rounded-lg cursor-pointer hover:shadow-md transition ${selectedCourse === course._id ? 'ring-2 ring-yellow-500' : 'border-gray-300'}`}>
                  <input
                    type="radio"
                    name="course"
                    value={course._id}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="lg:w-1/2 bg-white p-6 rounded-xl shadow">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block mb-1 text-sm font-medium">Full Name as per Aadhar</label>
              ...
              <div>
                <label className="block mb-1 text-sm font-medium">Confirm Email Address</label>
              ...
              <div>
                <label className="block mb-1 text-sm font-medium">Select State</label>
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <p className="text-sm">
                I agree with <a href="/terms-and-conditions" className="text-primary underline">Terms & Conditions</a> and{" "}
